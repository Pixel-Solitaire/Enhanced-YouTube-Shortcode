	...
	class pxsol_youtube_plugin {
		...
		function pxsol_youtube($atts) {
			...
			if ($pxsol_youtube_fullScreen == 'true'){
				$switchFullScreen = '&fs=1';
			} else {
				$switchFullScreen = '&fs=0';
			};
			$switchControls = '';
			if ($pxsol_youtube_controls == 'true'){
				$switchControls = '&controls=1';
			} else {
				$switchControls = '&controls=0';
			};
			$switchAutohide = '';
			if ($pxsol_youtube_autohide == 'true'){
				$switchAutohide = '&autohide=1';
			} else {
				$switchAutohide = '&autohide=0';
			};
			$switchModestbranding = '';
			if ($pxsol_youtube_modestbranding == 'true'){
				$switchModestbranding = '&modestbranding=1';
			} else {
				$switchModestbranding = '&modestbranding=0';
			};
			$pxsolUrlSwitch = $switchFullScreen.$switchControls.$switchAutohide.$switchModestbranding;
			//Code generation
			...
			<embed src="http://www.youtube.com/v/'.$id.'?version=3'.$pxsolUrlSwitch.'" type="application/x-shockwave-flash"	allowfullscreen="'.$pxsol_youtube_fullScreen.'" 
		...
		function pxsol_youtube_desc_links($links, $file) {
			...
			if ($file == $this_is_pxsol_youtube){
				...
				array_unshift($links, $pxsol_settings_link);
			};
			return $links;
		}
		...
		function pxsol_youtube_menu() {
			add_options_page( 
				...
				'manage_options', 
				__FILE__, 
				array($this,'pxsol_youtube_options')
			);
		}
		...
		<div id="pxsol_youtube" class="wrap">
			...
			<div class="pxsol_options pxsol_top"><div class="pxsol_bottom"><div class="pxsol_main">
				...
				if ($_REQUEST['submit']) {
					...
					<?php } else { ?>
						<div id="message" class="error fade">
							<p>Failed to save SettingsSettings <strong>:-(</strong></p>
						</div>
					<?php
					}
				}
				...
				<form method="post">
					<ul>
						...
						<li><label for="pxsol_youtube_modestbranding"><strong class="turquoise">Hide the YouTube logo from the bottom bar?</strong><br />
							<?php if ($default_youtube_modestbranding == 'true') { ?>
							<input class="radio" type="radio" name="pxsol_youtube_modestbranding" value="true" checked /> <strong>Yes</strong> (<i>actual setting</i>)<br />
							<input class="radio" type="radio" name="pxsol_youtube_modestbranding" value="false" /> No
							<?php } else { ?>
							<input class="radio" type="radio" name="pxsol_youtube_modestbranding" value="true" /> Yes<br />
							<input class="radio" type="radio" name="pxsol_youtube_modestbranding" value="false" checked /> <strong>No</strong> (<i>actual setting</i>)
							<?php }; ?>
						</label></li>
					<ul>
					<br />
			...
			<div class="pxsol_versions pxsol_top"><div class="pxsol_bottom"><div class="pxsol_main">
				<h3>Changelog</h3>
				<p class="dernier"><strong>v1.6</strong> Bugs repair / Hide YouTube logo / performance improvements<br />
		...
		function pxsol_admin_bar() {
			...
			$wp_admin_bar->add_menu( array( // Plugin button
...
// //////////////
if(class_exists("pxsol_youtube_plugin")) {
	$pixel_youtube_plugin = new pxsol_youtube_plugin;
}

// Hooks
// /////
register_activation_hook(__FILE__, array($pixel_youtube_plugin,"pxsol_youtube_set_options"));
add_shortcode('youtube_video', array($pixel_youtube_plugin,"pxsol_youtube"),1);
register_deactivation_hook(__FILE__, array($pixel_youtube_plugin,"pxsol_youtube_unset_options"));
add_action( 'wp_before_admin_bar_render', array($pixel_youtube_plugin,"pxsol_admin_bar"),1);
add_action( 'admin_init', array($pixel_youtube_plugin,"pxsol_youtube_init"),1);
add_filter('plugin_action_links', array($pixel_youtube_plugin,"pxsol_youtube_desc_links"), 10, 2 );
add_action('admin_menu', array($pixel_youtube_plugin,"pxsol_youtube_menu"),1);
?>